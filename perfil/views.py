...
class Criar(BasePerfil):
    def post(self, *args, **kwargs):
        ...
        if not self.userform.is_valid() or not self.perfilform.is_valid():
            ...
            messages.error(self.request, 'Existem erros no formulário de cadastro, por favor verifique se todos os campos estão corretos.')
        ...
        if password:
            ...
            if autentica:
                login_(self.request, user=usuario)

        self.request.session['carrinho'] = self.carrinho
        self.request.session.save()

        messages.success(self.request, 'Cadastro criado ou atualizado com sucesso.')
...
class Login(View):
    def post(self, *args, **kwargs):
        ...
        if not username or not password:
            messages.error(self.request, 'Usuaário ou senha inválidos.')
